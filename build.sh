...
ANDROID_STANDALONE_PATH="$CURRENTPATH/android-standalone-toolchain/"
...
: ${GCCARM_EXTRA_CPPFLAGS:="-DDBOOST_ASIO_DISABLE_STD_ATOMIC -DBOOST_AC_USE_PTHREADS -std=$CPPSTD"}
...
: ${ANDROIDBUILDDIR:=`pwd`/../build/libs/boost/lib}
...
: ${OUTPUT_DIR_LIB_ANDROID:=`pwd`/../libs/boost/android/}
...
cleanEverythingReadyToStart()
{
    ...
    rm -rf iphone-build iphonesim-build osx-build android-build
    rm -rf $IOSBUILDDIR
    rm -rf $PREFIXDIR
    rm -rf $IOSINCLUDEDIR
...
prepare()
{
    ...
    mkdir -p $OUTPUT_DIR_LIB_ANDROID
...
get_ndk()
{
	...
	echo "Make standalone Android Crossing Plafform Toolchain"
	if [ -d "$ANDROID_STANDALONE_PATH" ]; then
		echo "already there $ANDROID_STANDALONE_PATH, skip"
	else
		$NDK_ROOT/build/tools/make-standalone-toolchain.sh --platform=android-$ANDROID_API_LEVEL --toolchain=arm-linux-androideabi-4.9 --install-dir=$ANDROID_STANDALONE_PATH
		if [ $? != 0 ]; then 
			echo "Problem while making standalone android crossing platform"
			exit 1
		else
			echo "making android standalone crossing platfrom ok"
		fi
	fi
	ANDROID_COMPILER=$ANDROID_STANDALONE_PATH/bin/arm-linux-androideabi-g++
	ANDROID_AR=$ANDROID_STANDALONE_PATH/bin/arm-linux-androideabi-ar
	ANDROID_SYSROOT=$ANDROID_STANDALONE_PATH/sysroot
	if [ ! -s "$ANDROID_COMPILER" ]; then
		echo "cann't locate android crossing compiler"
		abort
	fi
	if [ ! -s "$ANDROID_AR" ]; then
		echo "cann't locate android crossing archiver"
		abort
	fi
	if [ ! -d "$ANDROID_SYSROOT" ]; then
		echo "$ANDROID_SYSROOT invalid, make sure your android api level is correct"
		abort
	fi
	doneSection
}
...
;
using gcc : arm
: $ANDROID_COMPILER -std=c++11 --sysroot=$ANDROID_SYSROOT
;
EOF
...
buildBoostForIPhoneOS()
{
    ...
    echo "------------------"
    LOG="$LOGDIR/build-android-build.log"
    echo "Running bjam for android-build "
    echo "To see status in realtime check:"
    echo " ${LOG}"
    echo "Please stand by..."
    ./bjam -j16 --build-dir=android-build -sBOOST_BUILD_USER_CONFIG=$BOOST_SRC/tools/build/example/user-config.jam --stagedir=android-build/stage --toolset=gcc-arm architecture=arm address-model=32 target-os=linux abi=aapcs binary-format=elf  link=static stage -d+2 > "${LOG}" 2>&1
    if [ $? != 0 ]; then 
        echo "Problem while Building android build - Please check ${LOG}"
        exit 1
    else 
    ...
    echo " ${LOG}"
    echo "Please stand by..."
    ./bjam -j16 --build-dir=iphonesim-build -sBOOST_BUILD_USER_CONFIG=$BOOST_SRC/tools/build/example/user-config.jam --stagedir=iphonesim-build/stage --toolset=darwin-${IPHONE_SDKVERSION}~iphonesim architecture=x86 address-model=32_64 target-os=iphone macosx-version=iphonesim-${IPHONE_SDKVERSION} link=static stage > "${LOG}" 2>&1
    if [ $? != 0 ]; then 
...
scrunchAllLibsTogetherInOneLibPerPlatform()
{
	...
	for NAME in $ALL_LIBS; do
		echo "Decomposing $ANME... for android"
		(cd $BOOST_SRC/android-build/stage/lib/; $ANDROID_AR x $NAME)
	done

	echo "Making Android Single Lib File"
	(cd $BOOST_SRC/android-build/stage/lib/; $ANDROID_AR qs libboost.a *.o; cp libboost.a $OUTPUT_DIR_LIB_ANDROID/)
...
echo "ANDROID_COMPILER"  $ANDROID_COMPILER
echo
